...
type Config struct {
	...
	// GeoJSONFilterPath, when set, loads polygons and drops map entries whose
	// coordinates fall outside all polygons (planar lon/lat test).
	GeoJSONFilterPath      string
	GeoJSONIncludeNoCoords bool
	GeoJSONFilter          *geojsonfilter.Filter
}
...
func ParseConfig() *Config {
	...
	flag.StringVar(&cfg.GeoJSONFilterPath, "geojson-filter", "", "path to GeoJSON Polygon/MultiPolygon (or Feature/FeatureCollection) to keep only results inside the boundary [default: off]")
	flag.BoolVar(&cfg.GeoJSONIncludeNoCoords, "geojson-include-missing-coords", false, "with -geojson-filter: pass through entries without valid coordinates instead of dropping them [default: false]")
	...
	if cfg.GeoJSONFilterPath != "" {
		f, err := geojsonfilter.LoadFile(cfg.GeoJSONFilterPath)
		if err != nil {
			fmt.Fprintf(os.Stderr, "invalid -geojson-filter %q: %v\n", cfg.GeoJSONFilterPath, err)
			os.Exit(1)
		}
